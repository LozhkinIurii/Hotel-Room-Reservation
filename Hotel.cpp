...
//  Checks if room is available or not (as separate option in menu).
...
// Making a room reservation subroutine
...
// Same as roomReservation subroutine, but room is drawn by program after user chooses room size
...
// Checking reservation info by booker name or by reservation number
...
void menu() {
	...
	//  Program draws how many rooms (even number) there are in the hotel
    do {
		even = rand() % 41 + 40;
		if (even % 2 == 0) {
			allRooms = even;
    ...
    // All rooms in beginning are available
	for (int i = 0; i < allRooms; i++) {
		reservation[i] = true;
	}
    // Everytime when returning to menu checks if all rooms are reserved
	do {
		for (int i = 0; i < allRooms; i++) {
...
// Main function. The menu is called from here.
int main() {
	srand(time(NULL));
	menu();
	return 0;
